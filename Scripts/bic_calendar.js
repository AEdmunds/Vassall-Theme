        ...
        this.each(function() {
            ...
            function marcarEventos(mes, ano) {
                ...
                for (var i = 0; i < events.length; i++) {
                    ...
                    if (events[0][i].date.split('/')[1] == t_mes && events[0][i].date.split('/')[2] == ano) {

                        $('#' + id_calendari + '_' + events[0][i].date.replace(/\//g, "_")).addClass('event');

                        $('#' + id_calendari + '_' + events[0][i].date.replace(/\//g, "_") + ' a').attr('data-original-title', events[i][1]);

                        //bg
                        if (events[0][i].color)
                            $('#' + id_calendari + '_' + events[0][i].date.replace(/\//g, "_")).css('background', events[0][i].color);

                        //link
                        if (events[0][i].link == '' || events[0][i].link == '#') {
                            if (events[0][i].htmlPopup != '') {
                                $('#' + id_calendari + '_' + events[0][i].date.replace(/\//g, "_") + ' a').attr('data-trigger', 'manual');
                                $('#' + id_calendari + '_' + events[0][i].date.replace(/\//g, "_") + ' a').addClass('manual_popover');
                            } else {
                                $('#' + id_calendari + '_' + events[0][i].date.replace(/\//g, "_") + ' a').attr('href', 'javascript:false;');
                            }
                        } else {
                            $('#' + id_calendari + '_' + events[0][i].date.replace(/\//g, "_") + ' a').attr('href', events[0][i].link);
                        }


                        //tooltip vs popover
                        if (events[0][i].htmlPopup) {
                            $('#' + id_calendari + '_' + events[0][i].date.replace(/\//g, "_")).addClass('event_popover');
                            $('#' + id_calendari + '_' + events[0][i].date.replace(/\//g, "_") + ' a').attr('rel', 'popover');
                            $('#' + id_calendari + '_' + events[0][i].date.replace(/\//g, "_") + ' a').attr('data-content', events[0][i].htmlPopup);
                        } else {
                            $('#' + id_calendari + '_' + events[0][i].date.replace(/\//g, "_")).addClass('event_tooltip');
                            $('#' + id_calendari + '_' + events[0][i].date.replace(/\//g, "_") + ' a').attr('rel', 'tooltip');
                        }
                    }
                }
